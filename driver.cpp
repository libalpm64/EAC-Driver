...
#define CODE_RW CTL_CODE(FILE_DEVICE_UNKNOWN, 0x901, METHOD_BUFFERED, FILE_ANY_ACCESS)  
#define CODE_BA CTL_CODE(FILE_DEVICE_UNKNOWN, 0x902, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define CODE_GET_GUARDED_REGION CTL_CODE(FILE_DEVICE_UNKNOWN, 0x903, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define CODE_GET_DIR_BASE CTL_CODE(FILE_DEVICE_UNKNOWN, 0x904, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define CODE_SECURITY 0x457c1d9
...
void HideDriver(PDRIVER_OBJECT DriverObject) {
    ULONG_PTR PiDDBLockPtr = 0;
    ULONG_PTR PiDDBCacheTablePtr = 0;
    PRTL_AVL_TABLE PiDDBCacheTable = (PRTL_AVL_TABLE)PiDDBCacheTablePtr;
    ExAcquireResourceExclusiveLite((PERESOURCE)PiDDBLockPtr, TRUE);
    ...
    ExReleaseResourceLite((PERESOURCE)PiDDBLockPtr);
}
...
NTSTATUS initialize_driver(_In_ PDRIVER_OBJECT drv_obj, _In_ PUNICODE_STRING path) {
    ...
    UNICODE_STRING name, link;
    RtlInitUnicodeString(&name, L"\\Device\\KeyboardClass0");  // Keyboard Driver (enh why not)
    RtlInitUnicodeString(&link, L"\\DosDevices\\KeyboardClass0");
